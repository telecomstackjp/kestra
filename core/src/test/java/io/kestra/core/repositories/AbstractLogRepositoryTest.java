...
public abstract class AbstractLogRepositoryTest {
    ...
    void findAsych() {
        ...
        Flux<LogEntry> find = logRepository.findAsynch(Pageable.UNPAGED, null, "io.kestra.unittest", Level.INFO, startDate);
        List<LogEntry> logEntries = find.collectList().block();
        assertThat(logEntries.size(), is(3));

        find = logRepository.findAsynch(Pageable.from(1, 2), null, null, Level.INFO, startDate);
        logEntries = find.collectList().block();
        assertThat(logEntries.size(), is(2));

        find = logRepository.findAsynch(Pageable.UNPAGED, null, null, Level.ERROR, startDate);
        logEntries = find.collectList().block();
        assertThat(logEntries.size(), is(1));

        find = logRepository.findAsynch(Pageable.UNPAGED, null, "io.kestra.unused", Level.INFO, startDate);
        logEntries = find.collectList().block();
        assertThat(logEntries.size(), is(0));

        find = logRepository.findAsynch(Pageable.UNPAGED, null, null, Level.INFO, startDate.plusSeconds(2));
        logEntries = find.collectList().block();
        assertThat(logEntries.size(), is(0));
    }
}
